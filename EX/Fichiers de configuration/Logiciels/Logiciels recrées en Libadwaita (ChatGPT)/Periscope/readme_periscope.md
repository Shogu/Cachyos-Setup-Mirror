...
Installation : 
```fish
sudo pacman -U ./periscope-0.1.0-4-any.pkg.tar.zst && shelly mark ignore periscope --add #empêche Shelly d'essayer de mettre à jour Periscope à partir des sources d'une appli AUR du même nom
```
...
```bash
sudo pacman -U ./periscope-0.1.0-7-any.pkg.tar.zst
```
