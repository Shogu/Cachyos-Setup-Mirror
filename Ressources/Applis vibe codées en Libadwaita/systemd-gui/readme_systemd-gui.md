...
![Interface de systemd-gui](../../screenshots/vibe-coded/systemd.png)
...
1. lit un plan détaillé avec `systemctl show` ;
2. affiche les fichiers locaux trouvés et les chemins protégés ;
3. indique les services qui référencent l’unité ;
4. demande une confirmation destructive ;
5. exige la saisie exacte du nom du service.

Lors de l’exécution, elle peut :

1. arrêter le service ;
2. le désactiver ;
3. le désmasquer si nécessaire ;
4. supprimer ses fichiers locaux et ses drop-ins ;
5. exécuter `systemctl daemon-reload` ;
6. exécuter `systemctl reset-failed`.
...
initial de l’interface. Elle reprend également la structure d’archive corrigée
de `0.3.1-2`, avec les métadonnées placées à la racine exacte du paquet.
Les archives `0.3.1-1` et `0.3.1-2` ne doivent pas être utilisées.
...
| --- | --- |
| 0.1.0 | Première interface native pour lister et inspecter les services. |
| 0.1.1 | Actions GUI visibles, panneaux de détail fiables et icônes compatibles avec les thèmes. |
| 0.2.0 | Correction des icônes et du chargement des panneaux ; état permanent du service ; ajout de la suppression complète et des unités fantômes. |
| 0.2.1 | Correction du callback de sélection ; rechargement réel des détails ; réactivation correcte des boutons après une suppression. |
| 0.2.2–0.2.3 | Suppression de l’onglet Propriétés redondant ; lecture du journal système avec repli Polkit ; recherche dynamique ; action principale adaptée à l’état du service. |
| 0.2.4 | Suppression du bouton Actualiser redondant dans le panneau droit ; recherche indépendante de la catégorie ; journal limité à vingt entrées ; retrait du libellé « Périmètre ». |
| 0.3.0 | Dépendances dans Vue d’ensemble ; coloration syntaxique ; barre bleue de sélection ; fond bleu-gris ; pastilles d’état ; compteur de résultats ; bouton Effacer. |
| 0.3.1 | Normalisation GNOME/libadwaita : couleurs adaptatives au thème, classe `.dimmed`, boutons `Adw.ButtonContent`, sorties monospace Adwaita et lignes de propriétés natives. |
| 0.3.1-2 | Reconstruction du paquet Arch avec les métadonnées Pacman à la racine exacte de l’archive. |
| 0.3.1-3 | Correction du démarrage de l’interface : `_configure_text_view()` reçoit correctement `self`. |
...
1. mettre à jour `APP_VERSION` dans `systemd.py` ;
2. mettre à jour `pkgver` dans `PKGBUILD` ;
3. ajouter une entrée de version dans `data/org.gnome.Systemd.appdata.xml` ;
4. ajouter la fonction ou la correction dans la section correspondante ;
5. compléter l’historique des versions ;
6. mettre à jour les noms des paquets dans les commandes d’installation ;
7. exécuter les vérifications ci-dessus ;
8. reconstruire l’archive source et le paquet Pacman.
